...
module.exports = {
  ...
  description:
    "Starts or stops a countdown timer - to which you can add to / subtract from.",
  args: true,
  usage:
    "timer [start|set|stop|end|add|plus|sub|subtract] [time in hh:mm:ss or mm:ss or mm format] [message to display]",
  execute(msg, args) {
...
function StartTimer(message, args) {
  ...
  if (!currentTimeObj) {
    message.channel.send(
      `The provided format seems to be invalid. Are you sure you entered hh:mm:ss or mm:ss or mm?`
    );
    return;
  }
  ...
  // set up timer
  var timeInSeconds =
    currentTimeObj.h * 3600 + currentTimeObj.m * 60 + currentTimeObj.s;
  if (timeInSeconds > 21600) {
    message.channel.send(
      `You tried to launch a timer for more than 6 hours. That's too much, sorry.`
    );
    return;
  }
  ...
  message.channel
    .send(`${txt}: ${setTimeDisplay(currentTimeObj, timeLeft)}`)
    .then(msg => {
      sentMsg = msg;
    })
    .catch(error => {
      message.channel.send(
        "Sorry, something went wrong. You can always try again, though."
      );
    });
  timer = setInterval(() => {
...
function setTimeDisplay(timeObject, seconds) {
  ...
  if (seconds >= 3600) {
    retStr =
      timeObject.h.toString().padStart(2, "0") +
      ":" +
      timeObject.m.toString().padStart(2, "0") +
      ":" +
      timeObject.s.toString().padStart(2, "0");
  } else if (seconds >= 60) {
    retStr =
      timeObject.m.toString().padStart(2, "0") +
      ":" +
...
function processInput(timeObj, timeInput) {
  ...
  if (timeInput.length == 1) {
    ...
    secs =
      parseInt(timeInput[0]) * 3600 +
      parseInt(timeInput[1]) * 60 +
      parseInt(timeInput[2]);
  } else {
    return null;
  }
